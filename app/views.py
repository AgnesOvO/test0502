...
#從MongoDB下載CSV檔(利用批次檔)
def export_files_from_mongodb(filename):
    conn = MongoClient("mongodb+srv://root:root159258@cluster0.oe4sl.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", tlsCAFile=certifi.where())
    db = conn.test #選擇操作 test 資料庫
    collection = db.test2 #選擇操作 users 集合
    # test if connection success
...
def test():
    a = pd.read_excel(app.config["EXCEL_UPLOADS"] + "/" + 'e1.xlsx')
    df = pd.DataFrame(a)
    List= df['總成績'].tolist()  
    print(List)
    ...
    for i in List:
        ...
        df.at[n, "總成績"] = 0 
        df = DataFrame(df) 
        DataFrame(df).to_excel(app.config["EXCEL_UPLOADS"] + "/" + 'e1.xlsx', sheet_name='Sheet1', index=False, header=True)
    return redirect("/download/"+'e1.xlsx') 
